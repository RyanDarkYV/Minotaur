...
namespace Minotaur.Operations.Services
{
    public class OperationsStorage : IOperationsStorage
    {
        ...
        public async Task<OperationDto> GetAsync(Guid id)
        {
            ...
            return string.IsNullOrWhiteSpace(operation) ? null : JsonConvert.DeserializeObject<OperationDto>(
                operation);
        }

        public async Task SetAsync(Guid id, Guid userId, string name, OperationState state, string resource, string code = null,
            string reason = null)
        {
