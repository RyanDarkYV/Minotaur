...
namespace Minotaur.CommonParts.RabbitMq
{
    public class BusSubscriber : IBusSubscriber
    {
        ...
        private async Task<Acknowledgement> TryHandleAsync<TMessage>(TMessage message,
            ...
            Func<Task> handle, Func<TMessage, MinotaurException, IRejectedEvent> onError = null)
        {
            var currentRetry = 0;
            ...
            return await retryPolicy.ExecuteAsync<Acknowledgement>(async () =>
            {
                ...
                using (scope)
                {
                    ...
                    catch (Exception exception)
                    {
                        ...
                        if (exception is MinotaurException minotaurException && onError != null)
                        {
                            var rejectedEvent = onError(message, minotaurException);
